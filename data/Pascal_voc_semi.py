...
class PascalVOCKeypoints(InMemoryDataset):
    ...
    def __init__(self, root, category, train_label=True, train_unlabel=False, transform=None, pre_transform=None, pre_filter=None):
    ...
    def processed_file_names(self):
        ...
        training_label: part of the original data.
        training_unlabel: all of the original data."""
        return ['training_label.pt', 'training_unlabel.pt', 'test.pt'] # todo: 多区分出一个label and unlabl？
    ...
    def process(self):
        ...
        train_unlabel_split = train_split = list(splits['train'])[category_idx]
        # shuffle
        random.shuffle(train_split)
        
        train_label_split = train_split[: len(train_split) //10]
        ...
        train_label_set, train_unlabel_set, test_set = [], [], []
        for i, name in enumerate(chain(train_label_split, train_unlabel_split, test_split)):
            ...
            for keypoint in keypoints:
                ...
                ys.append(labels[label]) # 相当于给 'name' encode, 成 0,1,2...
            ...
            if i < len(train_label_split):
                train_label_set.append(data)
            elif len(train_label_split) - 1 < i < len(train_label_split) + len(train_unlabel_split):
                train_unlabel_set.append(data)
            else:
                test_set.append(data)
            

        data_list = list(chain(train_label_set, train_unlabel_set, test_set))
        ...
        if self.pre_filter is not None:
            train_label_set = [data for data in train_label_set if self.pre_filter(data)]
            train_unlabel_set = [data for data in train_unlabel_set if self.pre_filter(data)]
            test_set = [data for data in test_set if self.pre_filter(data)]

        if self.pre_transform is not None:
            train_label_set = [self.pre_transform(data) for data in train_label_set]
            train_unlabel_set = [self.pre_transform(data) for data in train_unlabel_set]
            test_set = [self.pre_transform(data) for data in test_set]

        torch.save(self.collate(train_label_set), self.processed_paths[0])
        torch.save(self.collate(train_unlabel_set), self.processed_paths[1])
        torch.save(self.collate(test_set), self.processed_paths[2])
...
if __name__ == '__main__':
    ...
    path = osp.join('..', 'data', 'PascalVOC_SEMI_10')
    for category in PascalVOCKeypoints.categories:
        dataset = PascalVOCKeypoints(path, category, train_label=True, train_unlabel=False, pre_transform=pre_transform, pre_filter=pre_filter)
        dataset = PascalVOCKeypoints(path, category, train_label=False, train_unlabel=True, pre_transform=pre_transform, pre_filter=pre_filter)
        dataset = PascalVOCKeypoints(path, category, train_label=False, train_unlabel=False, pre_transform=pre_transform, pre_filter=pre_filter)
