# modified reference https://pytorch-geometric.readthedocs.io/en/latest/_modules/torch_geometric/datasets/willow_object_class.html#WILLOWObjectClass#
...
class WILLOWObjectClass(InMemoryDataset):
    ...
    def process(self):
        ...
        data_list = []
        for name in names:
            poss = loadmat(f'{name}.mat')['pts_coord']
            keypoints = []
            for i, pos in enumerate(np.split(poss, poss.shape[1], axis=1)):
                dict = {'name': i, 'pos': pos}
            ...
            data = Data(img=img, pos=pos, name=name, y=y)
...
if __name__ == '__main__':
    ...
    path = osp.join('..', 'data', 'WILLOW-ObjectClass_SSL')
    datasets = [WILLOWObjectClass(path, cat, pre_transform) for cat in WILLOWObjectClass.categories]
