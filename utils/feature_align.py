...
def feature_align(raw_feature: Tensor, P: Tensor, ns_t: Tensor, ori_size: tuple, device=None):
    ...
    :param raw_feature: raw feature map                         (batch_num * channel_num * feature_vector * feature_vector )
    :param P: point set containing point coordinates            (batch_size * ns_t * 2)
    :param ns_t: number of exact points in the point set 
    :param ori_size: size of the original image                 (PAIR.RESCALE: (256,256))
    ...
    for idx, feature in enumerate(raw_feature):                                             # feature = (channel_num * feature_vector * feature_vector)
        n = ns_t[idx]
        feat_size = torch.as_tensor(feature.shape[1:3], dtype=torch.float32, device=device) # feat_size = (channel_num * feature_vector) eg: 512 * 32 or  512 * 16
        _P = P[idx, 0:n]
        interp_2d(feature, _P, ori_size, feat_size, out=F[idx, :, 0:n])
        # F (batch_num * channel_num, n_max)
    return F
...
def interp_2d(z: Tensor, P: Tensor, ori_size: Tensor, feat_size: Tensor, out=None, device=None):
    ...
    step = ori_size / feat_size    # step = tensor([0.5, 8])
    if out is None:
        out = torch.zeros(z.shape[0], P.shape[0], dtype=torch.float32, device=device)
    for i, p in enumerate(P):
        # p = (p - step / 2) / ori_size * feat_size  # 更新keypoint坐标p #几何中心点对应关系：src_x = (des_x + 0.5) * src_w / des_w - 0.5 #TODO:
        p = (p + 0.5) * feat_size / ori_size - 0.5
...
def bilinear_interpolate_torch(im: Tensor, x: Tensor, y: Tensor, out=None, device=None):
    ...
    x0 = torch.floor(x) # x0 = 小于x的最大整数
    x1 = x0 + 1
    y0 = torch.floor(y)
    y1 = y0 + 1

    x0 = torch.clamp(x0, 0, im.shape[2] - 1) # 将元素夹在[min, max]之间, 这里将元素夹在[0, im.shape[2]-1 (31 or 15)]
