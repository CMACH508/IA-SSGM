...
class Voting(nn.Module):
    ...
    """
    def __init__(self, alpha=2000, pixel_thresh=None):
    ...
    def forward(self, s, nrow_gt, ncol_gt=None):
        ...
        self.W = Parameter(torch.ones_like(s))
        stdv = 1. / math.sqrt(1024)
        self.W.data += torch.zeros_like(s).uniform_(-stdv, stdv)
        for b, n in enumerate(nrow_gt):
            if ncol_gt is None:
                ret_s[b, 0:n, :] = self.softmax(torch.mul(self.W, self.alpha * s[b, 0:n, :]))
            else:
                ret_s[b, 0:n, 0:ncol_gt[b]] = self.softmax(torch.mul(self.W[b, 0:n, 0:ncol_gt[b]], self.alpha * s[b, 0:n, 0:ncol_gt[b]]))
