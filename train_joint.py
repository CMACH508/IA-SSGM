...
def train_eval_model(model, 
    ...
    # lam_loss_gcl = cfg.TRAIN.LOSS_GCL
    # lam_loss_perm = cfg.TRAIN.LOSS_PERM
    # lam_loss_consistency = cfg.TRAIN.LOSS_CONSISTENCY
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        lam_loss_perm = lam_loss_consistency =torch.tanh(torch.tensor(epoch / 100))
        lam_loss_gcl = torch.tensor(1) - lam_loss_perm 
        # lam_loss_perm = 1.0
        # lam_loss_consistency = 0.0
        # lam_loss_gcl = 0.0 
        ...
        for data in train_loader:
            ...
            with torch.autograd.set_detect_anomaly(True):
                z1_s, z2_s, g1_s, g2_s, z1_t, z2_t, g1_t, g2_t, pred_softmax_m, pred_permutation_m, s_num_node, t_num_node, probability, cross_probability = model(data.x_s, data.edge_index_s, data.edge_attr_s, data.x_s_batch, data.x_t, data.edge_index_t, data.edge_attr_t, data.x_t_batch)
                ground_truth_m = generate_y(data.y, data.x_s_batch, s_num_node, t_num_node).to(device)
                loss_gcl = criterion_gcl(h1 = torch.cat((z1_s, z1_t), axis=0), 
                                         h2 = torch.cat((z2_s, z2_t), axis=0), 
                                         g1 = torch.cat((g1_s, g1_t), axis=0), 
                                         g2 = torch.cat((g2_s, g2_t), axis=0),
                                         batch = torch.cat((data.x_s_batch, data.x_t_batch + data.x_s_batch.max() + 1), axis=0)).to(device)
                
                loss_perm = criterion_perm(pred_softmax_m, pred_permutation_m, s_num_node, t_num_node).to(device)
                loss_consistency = criterion_consistency(probability, cross_probability)
                loss = lam_loss_gcl * loss_gcl + lam_loss_perm * loss_perm + lam_loss_consistency * loss_consistency
                ...
                #     lam_loss_gcl = 0.0
                #     lam_loss_perm = 1.0
                #     lam_loss_consistency = 1.0

                # loss
                loss_dict = dict()
                loss_dict['loss'] = loss.item()
                loss_dict['loss_perm'] = loss_perm.item()
                loss_dict['loss_consistency'] = loss_consistency.item()
                loss_dict['loss_gcl'] = loss_gcl.item()
...
if __name__ == '__main__':
    args = parse_args('Joint Matching')
    ...
    #     train_datasets += [SamePairDataset(dataset, dataset, train=True, random=True)]
    ...
    # for category in datasets.categories:
    #     dataset = datasets(cfg.DATASET_PATH, category, train=True, pre_transform=pre_transform, pre_filter=pre_filter)
    #     train_datasets += [ValidPairDataset(dataset, dataset, sample=False, random=True)]
    #     dataset = datasets(cfg.DATASET_PATH, category, train=False, pre_transform=pre_transform, pre_filter=pre_filter)
    ...
    # datasets = [datasets(cfg.DATASET_PATH, cat, pre_transform) for cat in datasets.categories]
    # datasets = [dataset.shuffle() for dataset in datasets]
    # train_datasets = [dataset[:70] for dataset in datasets]
    ...
    criterion_perm = CrossEntropyLoss()
    criterion_consistency = nn.CrossEntropyLoss(reduction='mean')
    criterion_gcl = DualBranchContrast(loss=L.InfoNCE(tau=0.2), mode=cfg.TRAIN.LOSS_FUNC_MODE)
    ...
    with DupStdoutFileManager(str(Path(cfg.OUTPUT_PATH) / ('train_log_' + now_time + '.log'))) as _:
        print_easydict(cfg)
        model = train_eval_model(model, criterion_gcl, criterion_perm, criterion_consistency,optimizer, train_loader, test_datasets, tfboardwriter, start_epoch=cfg.TRAIN.START_EPOCH, num_epochs=cfg.TRAIN.NUM_EPOCHS, xls_wb=wb)
